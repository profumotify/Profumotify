// ============================================================
// PROFUMOTIFY v7.1 - APP JAVASCRIPT
// Collezione di Giancarlo - Bari
// Login utenti + link acquisto Fragrantica/Notino/Pinalli
// ============================================================
...
function init() {
  ...
  if (user) {
    ...
    if (userBadge) userBadge.textContent = `\u{1F464} ${user.name} - ${user.location}`;
  }
...
function showLoginScreen() {
  ...
  app.innerHTML = `
    <div class="login-screen">
      <div class="login-box">
        <h1>\u{1F489} Profumotify v7.1</h1>
        <p class="login-subtitle">Tracker personale per la collezione di profumi</p>
        <div class="login-form">
          <input type="text" id="login-user" placeholder="Username" value="giancarlo">
          <input type="password" id="login-pass" placeholder="Password" value="">
          <button onclick="doLogin()">Accedi</button>
          <p class="login-hint">Utenti: giancarlo / ospite<br>Password: bari2024 / ospite</p>
        </div>
      </div>
    </div>
  `;
...
// ============================================================
// MODAL DETTAGLI - CON LINK ACQUISTO
// ============================================================
function openModal(id) {
  ...
  modalBody.innerHTML = `
    ...
    <div class="buy-links">
      <h4>\u{1F6D2} Dove acquistare</h4>
      <div class="buy-buttons">
        <a href="${escapeHtml(p.fragrantica)}" target="_blank" class="buy-btn fragrantica">
          \u{1F4D6} Fragrantica
        </a>
        <a href="${escapeHtml(p.notino)}" target="_blank" class="buy-btn notino">
          \u{1F6CD}\uFE0F Notino
        </a>
        <a href="${escapeHtml(p.pinalli)}" target="_blank" class="buy-btn pinalli">
          \u{1F3EA} Pinalli
        </a>
      </div>
    </div>
